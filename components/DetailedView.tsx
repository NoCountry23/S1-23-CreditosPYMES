...
export default function DetailedView({
  ...
  const details = detailedRequestInfo
  ...
  const handleApprove = (requestId: string) => {
    if (window.confirm('¿Está seguro de aprobar esta solicitud?')) {
      alert(`Solicitud ${requestId} aprobada. El cliente será notificado.`)
      setSelectedRequest(null)
    }
  }
  ...
  const handleReject = (requestId: string) => {
    ...
    if (reason) {
      alert(
        `Solicitud ${requestId} rechazada. El cliente será notificado con el motivo.`
      )
      setSelectedRequest(null)
    }
  }
    ...
    <div className='fixed inset-0 bg-black bg-opacity-50 z-50 overflow-y-auto'>
      <div className='min-h-screen px-4 py-8'>
        <div className='max-w-6xl mx-auto bg-background  rounded-xl shadow-2xl'>
          {/* Header */}
          <div className='p-6 border-b  flex items-center justify-between bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-t-xl'>
            <div>
              <h2 className='text-2xl font-bold'>Revisión de Solicitud</h2>
              <p className='text-blue-100 mt-1'>{request.id}</p>
            </div>
            <button
                  ...
                  <div className='grid grid-cols-2 gap-4 text-sm'>
                    <div>
                      ...
                      <p className='font-medium'>
                        {details.companyInfo.legalName}
                      </p>
                    </div>
                    <div>
                      <p className='text-gray-600'>Nombre Comercial</p>
                      <p className='font-medium'>
                        {details.companyInfo.tradeName}
                      </p>
                    </div>
                    <div>
                      <p className='text-gray-600'>CUIT</p>
                      <p className='font-medium'>{details.companyInfo.cuit}</p>
                    </div>
                    <div>
                      <p className='text-gray-600'>Teléfono</p>
                      <p className='font-medium'>{details.companyInfo.phone}</p>
                    </div>
                  ...
                  <div className='grid grid-cols-2 gap-4'>
                    <div className=' p-4 rounded-lg'>
                      ...
                      <p className='text-2xl font-bold text-green-700'>
                        ${details.financialData.annualRevenue.toLocaleString()}
                      </p>
                    </div>
                    <div className=' p-4 rounded-lg'>
                    ...
                    <div className=' p-4 rounded-lg'>
                      ...
                      <p className='text-2xl font-bold text-purple-700'>
                        {details.financialData.employees}
                      </p>
                    </div>
                    <div className=' p-4 rounded-lg'>
                      <p className='text-sm text-gray-600'>
                      ...
                      <p className='text-2xl font-bold text-orange-700'>
                        {details.financialData.yearsInBusiness}
                      </p>
                    </div>
                    <div>
                      ...
                      <div className='flex items-center gap-2 mt-1'>
                        <div className='flex-1  rounded-full h-2'>
                          <div
                            ...
                            style={{
                              width: `${
                                (details.financialData.creditScore / 900) * 100
                              }%`,
                            }}
                          ></div>
                        </div>
                    ...
                    <div className='grid grid-cols-2 gap-4'>
                      <div>
                        ...
                        <p className='text-2xl font-bold text-blue-600'>
                          ${details.loanDetails.amount.toLocaleString()}
                        </p>
                      </div>
                      <div>
                        <p className='text-sm text-gray-600'>Plazo</p>
                        <p className='text-2xl font-bold'>
                          {details.loanDetails.term} meses
                        </p>
                      </div>
                      <div>
                        <p className='text-sm text-gray-600'>
                        ...
                        <div className='flex-1 bg-gray-200 rounded-full h-2 mr-3'>
                          <div
                            className={`h-2 rounded-full ${
                              details.riskAnalysis.debtToIncome < 0.4
                                ? 'bg-green-600'
                                : 'bg-yellow-600'
                            }`}
                            style={{
                              width: `${
                                details.riskAnalysis.debtToIncome * 100
                              }%`,
                            }}
                          ></div>
                        </div>
