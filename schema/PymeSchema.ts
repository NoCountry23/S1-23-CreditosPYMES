...
export const PymeSchema = z.object({
  ...
  .max(255, { message: "El email no puede exceder los 255 caracteres." }),
    ...
    legal_address: z.string().trim().min(1, { message: "La dirección legal no debe estar vacía." }).max(255),

    // city (character varying)
    city: z.string().trim().min(1, { message: "La ciudad no debe estar vacía." }).max(100),

    // local_state (character varying)
    local_state: z.string().trim().min(1, { message: "El estado local no debe estar vacío." }).max(100),
